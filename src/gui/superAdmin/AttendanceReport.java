...
public class AttendanceReport extends javax.swing.JDialog {
    ...
    private void loadTable(String date) {
        try {
            ResultSet resultSet = MySQL2.executeSearch("SELECT * FROM `attendance` INNER JOIN `employee` ON `employee`.`email`=`attendance`.`employee_email` INNER JOIN `attendstatus` "
                    + "ON `attendstatus`.`id` =`attendance`.`attendStatus_id` INNER JOIN `employee_type` ON `employee_type`.`id`=`employee`.`employee_type_id` WHERE `attendance`.`date` = '"+date+"' ");
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        try {
            ...
            params.put("Parameter1", formattedDate);
            
            //View or print report
            InputStream inputSteam = this.getClass().getResourceAsStream("/reports/attendance.jasper");
